...
const ImportMode = () => {
    ...
    () =>
      report
        ? [
            ...
            ['Tags créés', report.tagsCreated],
            ['Produits supprimés', report.productsDeleted],
            ['Images uploadées', report.imagesImported],
          ]
        : [],
    [report],
  );
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.append('expandMatrix', String(expandMatrix));

      const { post } = getFetchClient();
      const response = await post('/api/import/mode', formData);
    ...
    <main style={{ padding: 32, background: '#f6f6f9', minHeight: '100vh' }}>
      <div style={{ maxWidth: 1120, margin: '0 auto' }}>
        <header style={{ marginBottom: 24 }}>
          ...
          <h1 style={{ color: '#212134', fontSize: 32, margin: '6px 0 8px' }}>Import CSV Mode</h1>
          <p style={{ color: '#666687', fontSize: 16, margin: 0 }}>
            Dépose l’export WooCommerce (<code>wc-product-export-*.csv</code>) ou le CSV enrichi. L’import crée les
            produits, variantes (Top / Ensemble), <strong>couleurs</strong>, <strong>tailles</strong>, prix et stocks.
          </p>
        </header>
        ...
        <section style={cardStyle}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 18, margin: '20px 0' }}>
              ...
              <label style={{ alignItems: 'center', display: 'flex', gap: 8 }}>
                ...
                Télécharger les images dans la Media Library
              </label>

              <label style={{ alignItems: 'center', display: 'flex', gap: 8 }}>
                <input
                  checked={replaceCategory}
                  onChange={(event) => setReplaceCategory(event.target.checked)}
                  type="checkbox"
                />
                Remplacer toute la catégorie du CSV
              </label>

              <label style={{ alignItems: 'center', display: 'flex', gap: 8 }}>
                <input checked={expandMatrix} onChange={(event) => setExpandMatrix(event.target.checked)} type="checkbox" />
                Matrice complète type × couleur × taille
              </label>
            </div>

            {expandMatrix ? (
              <div style={{ background: '#eaf5ff', borderRadius: 8, color: '#0c447c', marginBottom: 20, padding: 14 }}>
                Crée une variante SKU pour chaque combinaison (ex. Top / Noir / M). Sinon : variantes = types tarifés, et
                couleurs/tailles sont stockées sur le produit.
              </div>
            ) : null}

            {replaceCategory ? (
              <div style={{ background: '#fff4e5', borderRadius: 8, color: '#7c4a03', marginBottom: 20, padding: 14 }}>
                Attention : supprime définitivement les produits et variantes de la catégorie détectée (ex. Mode) avant de
                réimporter.
              </div>
            ) : null}

            <p style={{ color: '#666687', fontSize: 14, marginBottom: 16 }}>
              Par défaut, les images restent en <strong>URL externe</strong> (<code>imageUrl</code>). Coche « Media Library »
              seulement si tu veux aussi les uploader sur le serveur.
            </p>
          ...
          {error ? (
            <div style={{ background: '#fff0f0', borderRadius: 8, color: '#b72b1a', marginTop: 20, padding: 14 }}>
              {error}
            </div>
          ) : null}
        </section>
